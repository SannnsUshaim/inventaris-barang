<x-app-layout>
    ...
    <div class="bg-white dark:bg-gray-800 flex flex-col w-full h-full relative rounded-md shadow-md p-6 justify-between">
            <div class="flex flex-col gap-3">
                <div class="flex">
                    <a href="{{ route('pemakaian.create') }}" class="bg-teal-500 px-4 py-1 text-white font-medium text-sm rounded-md flex items-center gap-2">Tambah Data Pemakaian 
                ...
                <table class="text-sm font-medium rounded-t-md">
                    <thead class="bg-slate-800 dark:bg-white">
                        <tr class="rounded-t-md">
                            ...
                            <th class="font-medium py-1 text-white dark:text-gray-900 text-left px-2">Nama Pemakai</th>
                            <th class="font-medium py-1 text-white dark:text-gray-900 text-left px-2">Tanggal</th>
                            ...
                            <th class="font-medium py-1 text-white dark:text-gray-900 text-left px-2">Total</th>
                    ...
                    <tbody>
                        @forelse ($pemakaian as $data)
                            <tr class="border-b-2">
                                ...
                                <td class="font-regular dark:text-white p-2">{{ \Carbon\Carbon::parse($data->tanggal)->format('d/m/Y') }}</td>
                                ...
                                <td class="font-regular dark:text-white p-2">Rp. {{ number_format($data->harga * $data->jumlah, 0 , '.', ',') }}</td>
                                <td>
                                    <form onsubmit="return confirm('Apakah Anda Yakin?');" action="{{ route('pemakaian.destroy', $data->id) }}" class="flex items-center gap-2" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit">
            ...
            <div class="text-sm dark:text-white">
                <p>total data: {{ $pemakaian->count() }}</p>
            </div>
        </div>
</x-app-layout>